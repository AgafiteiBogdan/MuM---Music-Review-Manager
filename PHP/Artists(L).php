...
<div class="sidebar" id="sidebar">
  ...
  <a href="Artists(L).php" class="active"><i class="fa fa-microphone" id="image"></i>Artists</a>
  ...
  <div>
   <form style="width:1500px;" method="POST">
     <select class="custom-select"  name="artist" id="gen" required="required">
      <option>Choose an artist:</option>
      <?php
       $sql = mysqli_query($db, "SELECT DISTINCT name FROM `artists` order by name" );
...
if(isset($_POST['test'])) {
   ...
   $sql =  "SELECT a.id as 'id', a.name as 'song', b.name as 'artist', c.name as 'album', c.release_year as 'year', a.genre as 'genre'from artists b join albums c on b.id=c.artist_id  join Songs a on c.id = a.album_id order by a.id, b.name, c.name";
     ...
     <?php
   $sql =  "SELECT a.id as 'id', a.name as 'song', b.name as 'artist', c.name as 'album', c.release_year as 'year', a.genre as 'genre'from artists b join albums c on b.id=c.artist_id  join Songs a on c.id = a.album_id where b.name = '$option' order by  b.name, c.name";
...
<div class="modal-content">
    ...
    <span onclick = "closeModal(<?php echo $poz?>)" class="close" >&times;</span>
      <?php $poz = $poz + 1;?>
    <div class="container">
  <form action="Artists(L).php" method="post">
